...
export default {
  ...
  methods: {
    async setDataInChart(){
      ...
      let region = "";
      let mode = "ine";
      let i = API_INFO[this.$route.params.id]["Compare"].cod[this.selectedFeature];
      if (API_INFO[this.$route.params.id]["Compare"].cod[this.selectedFeature].substring(0,6) == 'COVID-'){
        region = this.$route.params.id + "/";
        mode = "covid";
        i = API_INFO[this.$route.params.id]["Compare"].cod[this.selectedFeature].substring(6);
      }
      ...
      if (this.selectedRegion != null ){
        ...
        let region = "";
        let mode = "ine";
        let i = API_INFO[this.selectedRegion]["Compare"].cod[this.selectedFeature];
        if (API_INFO[this.selectedRegion]["Compare"].cod[this.selectedFeature].substring(0,6) == 'COVID-'){
          region = this.selectedRegion + "/";
          mode = "covid";
          i = API_INFO[this.selectedRegion]["Compare"].cod[this.selectedFeature].substring(6);
        }
  ...
  watch: {
    selectedRegion(){ this.setDataInChart()
    },
    selectedFeature(){
      this.setDataInChart();
    } 
